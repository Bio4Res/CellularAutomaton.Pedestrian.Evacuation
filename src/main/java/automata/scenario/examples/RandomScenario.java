...
public class RandomScenario {
  public static Scenario randomScenario() {
    ...
    // place exits
    if (Random.random.bernoulli(0.9)) {
    ...
    if (Random.random.bernoulli(0.5)) {
      scenario.setExit(new Rectangle(rows / 2, columns / 2, 2, 2));
    }

    // place blocks
    int numberOfBlocks = Random.random.nextInt(50, 120);
    int numberOfBlocksPlaced = 0;
    int maxTries = numberOfBlocks * 3;
    while (numberOfBlocksPlaced < numberOfBlocks && maxTries > 0) {
      var width = Random.random.bernoulli(0.5) ? 1 + Random.random.nextInt(2) : 1 + Random.random.nextInt(20);
      var height = 1 + Random.random.nextInt(Math.max(1, rows / (2 * width)));

      var row = Random.random.nextInt(0, 1 + rows - height);
      var column = Random.random.nextInt(2, 1 + columns - width - 2);
